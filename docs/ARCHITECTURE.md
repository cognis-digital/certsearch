...
- **rules/analyzers** apply the heuristics shipped in `certsearch/core.py`.
- **score** ranks by severity.
- **MCP server** (`certsearch mcp`) exposes `scan` for Cognis.Studio agents.
